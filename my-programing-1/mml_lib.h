...
// 近似sin（5次多項式）: xは[-PI,PI]に正規化してから使用
double sin5(double x){
  while(x> PI) {x-=PI*2.0;}
  while(x<-PI) {x+=PI*2.0;}
  if (x> PI*0.5) {x= PI-x;} // これを追加
  if (x<-PI*0.5) {x=-PI-x;} // これを追加
  double x2=x*x;
...
void mml_to_wav(const char *MML, const char *WAV_PATH){
  ...
  int volume=8;
  double baseA4=440.0;
  int waveform=1; // @0..5: 0=sin,1/2/3=pulse,4=triangle,5=saw
  ...
  while(*p){
    ...
    if(c=='Q'){ p++; double v=read_number(&p); if(v>0) gate8=(int)v; continue; } // Qを追加
