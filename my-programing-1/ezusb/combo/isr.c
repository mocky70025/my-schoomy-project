...
extern const BYTE ReportDscr_Mouse[];
extern const BYTE ReportDscrSize_Mouse[];
extern const BYTE ReportDscr_Keyboard[];
extern const BYTE ReportDscrSize_Keyboard[];
  ...
  if (USBIRQ & b0) { // SETUP
    if (SETUPDAT[1] == 6) { // GET_DESCRIPTOR
      ...
      case 0x22: // HID_REPORT
        if (SETUPDAT[4] == 0) {
          hid_report(ReportDscr_Mouse, ReportDscrSize_Mouse);
        } else if (SETUPDAT[4] == 1) {
          hid_report(ReportDscr_Keyboard, ReportDscrSize_Keyboard);
        } else {
          EP0CS |= b0; // STALL
        }
        break;
